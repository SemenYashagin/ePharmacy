Public Class Orders
    ...
    Private Function ToDataTable(recipes As eRecipe) As DataTable
        ...
        column = New DataColumn With {
            ...
            .Caption = "Кол-во товара (упаковок)",
            .[ReadOnly] = True
        }
        table.Columns.Add(column)
        Try
            For Each order As Order In recipes.orders
                For Each product As Product In order.products
                    ...
                    If (IsNothing(product.packageContent)) Then row(4) = "" Else row(4) = product.packageContent.amount.ToString() + " " + product.packageContent.unit
